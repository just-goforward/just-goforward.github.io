...
- 계산 방식 선택지를 추가했습니다.
  - `단일 목표`: 이 소장품만 SR 15까지 보낼 때, SR 15 도달 확률을 가능한 유지하면서 키트의 소모량을 줄입니다.
  - `수급량 고려`: 이 소장품의 SR 15 도달 확률을 크게 깎지 않는 선에서, 수급량이 적은 키트를 더 아낍니다.
- MDP 정책 기준을 `SR 15 도달 확률 최대화`에서 `도달 확률 보존 + 소모 효율` 중심으로 조정했습니다.
- 수급량 고려 계산에 28일 기대 획득량 기준을 반영했습니다.
  - 초심자용 관리 키트: 473.912개
  - 중급자용 관리 키트: 55.808개
  - 상급자용 관리 키트: 24.736개
- 수급량 고려 모드의 SR 15 도달 확률 허용폭을 0.5%p로 조정했습니다.
