(function () {
  ...
  const { KIT_ORDER, KIT_META, FIXED_REQUIRED_EXP, STRATEGY_META, describeState, transition, normalizeState, convertState } =
  ...
  function formatPercentagePoint(value, digits = 2) {
    return `${formatNumber(value * 100, digits)}%p`;
  }
  ...
  function queueStatsEvent(event) {
    ...
    submitStatsEvent(event).catch((error) => {
      // Statistics are optional and must never block the calculator.
      ...
      <div class="result-content">
        <div class="stats-grid">
          ${byKit
            .map(
              (item) => `
                <div class="stats-card ${kitClass(item.kit)}">
                  <span><i></i>${kitName(item.kit)}</span>
                  <strong>${formatPercent(item.greatSuccessRate || 0, 1)}</strong>
                  <small>${formatInteger(item.events || 0)}건 중 대성공 ${formatInteger(item.greatSuccesses || 0)}건</small>
                </div>
              `,
            )
            .join("")}
  ...
  async function refreshGlobalStats() {
    if (!statsApiBase() || !globalStatsPanel) return;
    try {
      ...
      // Leave the optional stats panel hidden when the endpoint is unavailable.
    }
  }
  ...
  function validateInWorker(input, runs, onProgress) {
    return requestWorkerTask("validate", input, { payload: { runs }, onProgress });
  }
  ...
  function validateBestAvailable(input, runs, onProgress) {
    const key = `${calculationInputKey(input)}|mc:${runs}`;
    if (validationCache.has(key)) return Promise.resolve(validationCache.get(key));
    const workerPromise = validateInWorker(input, runs, onProgress);
    if (workerPromise) {
      return workerPromise
        ...
        .catch(() => {
          ...
          const result = window.CollectionSolver.solve({ ...input, monteCarloRuns: runs }, onProgress).monteCarlo;
          return rememberCache(validationCache, key, result, VALIDATION_CACHE_LIMIT);
        });
    }
    return Promise.resolve(
      rememberCache(validationCache, key, window.CollectionSolver.solve({ ...input, monteCarloRuns: runs }, onProgress).monteCarlo, VALIDATION_CACHE_LIMIT),
    );
  }
  ...
  function strategyCostLabel(strategy) {
    return strategy === "supply" ? "복합 비용" : "보유량 압박";
  }

  function candidateStrategyCost(candidate, strategy) {
    return strategy === "supply" ? candidate.resourceCost : candidate.pressure;
  }

  function candidateRows(candidates, strategy) {
    return candidates
      .map(
        (candidate) => `
          <tr>
            ...
            <td>${formatPercentagePoint(candidate.probabilityGap || 0, 2)}</td>
            <td>${formatNumber(candidateStrategyCost(candidate, strategy), 4)}</td>
          </tr>
        `,
      )
      .join("");
  ...
  function renderResult(result) {
    ...
    const supplyWeights = (result.stats && result.stats.supplyWeights) || { supply: 0.75, stock: 0.25 };
    const strategyBasis =
      strategy === "supply"
        ? `수급량 고려 모드는 28일 기대 획득량 기준 비용 ${formatPercent(supplyWeights.supply, 0)}와 현재 보유량 압박 ${formatPercent(supplyWeights.stock, 0)}를 합산한 복합 비용을 사용합니다.`
        : "단일 목표 모드는 현재 보유량 대비 소모 압박을 기준으로 특정 키트가 과하게 줄어드는 선택을 피합니다.";
    ...
    detailBox.innerHTML = `
      <div class="result-content">
        ...
        <dl class="detail-list">
          ...
          <div>
            <dt>계산 방식</dt>
            <dd>벨만 방정식 기반의 유한 MDP로, 현재 소장품 상태와 남은 키트 사용 가능 횟수를 함께 평가합니다. 정확한 최대 SR 15 도달 확률에서 ${formatPercentagePoint(result.stats.probabilityTolerance || 0, 1)} 이내인 후보만 남긴 뒤, ${strategy === "supply" ? "수급량과 보유량을 함께 반영한 복합 비용이 낮은" : "보유량 압박과 기대 소모가 낮은"} 행동을 선택합니다. ${strategyBasis} 상태별 worst-case 사용 횟수로 SR 15 전에 소비될 수 없는 초과 재고는 탐색에서 제외하며, 이상 계산 시 기존 재고로 되돌리는 안전장치를 둡니다. 추천 행동은 정확 전이 계산으로 결정되고, 가상의 니붕이 검증은 별도로 실행하는 보조 확인 기능입니다.</dd>
          </div>
        </dl>
        ...
        <details class="validation-details">
          ...
          <div class="validation-content">
            <p>
              가상의 니붕이 ${monteCarloRuns().toLocaleString("ko-KR")}명을 동일한 조건에서 시도하게 해, 계산 결과와 직관적으로 맞는지 확인하는 기능입니다.
            </p>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>SR 15 도달 확률</th>
                <th>최대 대비 차이</th>
                <th>${strategyCostLabel(strategy)}</th>
              </tr>
            </thead>
            <tbody>${candidateRows(result.topCandidates, strategy)}</tbody>
          </table>
        </div>
      </div>
    `;
  ...
  async function runMonteCarloValidation(button) {
    ...
    try {
      ...
      if (output) {
        output.textContent = `가상의 니붕이 ${monteCarlo.runs.toLocaleString("ko-KR")}명 중 ${formatPercent(
