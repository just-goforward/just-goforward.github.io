...
return strategy === "supply" ? "수급 비용" : "보유량 압박";
}
function candidateStrategyCost(candidate, strategy) {
return strategy === "supply" ? candidate.supplyCost : candidate.pressure;
}
...
const strategyBasis =
strategy === "supply"
? "수급량 고려 모드는 28일 기대 획득량 기준으로 초심자 473.912개, 중급자 55.808개, 상급자 24.736개를 사용해 희소한 키트의 비용을 더 크게 계산합니다."
...
<div>
<dt>계산 방식</dt>
<dd>보유 키트 사용 가능 횟수를 상태에 포함한 유한 MDP입니다. 정확한 최대 SR 15 도달 확률에서 ${formatPercentagePoint(result.stats.probabilityTolerance || 0, 1)} 이내인 후보만 남긴 뒤, ${strategy === "supply" ? "수급 비용과 보유량 압박이 낮은" : "보유량 압박과 기대 소모가 낮은"} 행동을 선택합니다. ${strategyBasis} 확률 전이는 정확 계산이며 반복 수렴 허용오차는 0입니다.</dd>
</div>
</dl>
