...
<h3>대성공 시도 분포</h3>
${attemptDistribution(stats.successAttemptDistribution || [])}
</section>
<section class="stats-section stats-section-wide">
<h3>이론 vs 실측 대성공률</h3>
${levelTheoryTable(stats.levelKitStats || [])}
</section>
<section class="stats-section">
<h3>구간별 체감 난이도</h3>
...
const summary = stats && stats.summary ? stats.summary : {};
const mostUsedKit = summary.mostUsedKit ? `${kitName(summary.mostUsedKit)}(${formatInteger(summary.mostUsedKitPieces || 0)}개)` : "-";
return `
<section class="stats-section">
<h3>누적 참여 현황</h3>
<div class="stats-overview">
<span>총 <b>${formatInteger(summary.attempts || 0)}회</b> 집계</span>
<span>오늘 <b>${formatInteger(summary.todayAttempts || 0)}회</b></span>
<span>30일간 가장 많이 사용된 키트: <b>${mostUsedKit}</b></span>
</div>
</section>
`;
}
function attemptDistribution(rows) {
if (!rows.length) return `<p class="stats-empty">아직 대성공 시도 분포가 없습니다.</p>`;
const grouped = new Map();
for (const row of rows) {
const attempt = Number(row.successAttempt || 0);
grouped.set(attempt, (grouped.get(attempt) || 0) + Number(row.events || 0));
}
const bars = [...grouped.entries()].sort((a, b) => a[0] - b[0]);
const total = bars.reduce((sum, [, events]) => sum + events, 0);
const maxEvents = Math.max(...bars.map(([, events]) => events), 1);
return `
<div class="attempt-histogram" aria-label="대성공 시도 분포">
${bars
.map(([attempt, events]) => {
const height = Math.max(6, (events / maxEvents) * 100);
const ratio = total > 0 ? events / total : 0;
return `
<div class="attempt-bar">
<em>${formatPercent(ratio, 0)}</em>
<span style="height:${height}%"></span>
<b>${formatInteger(attempt)}회</b>
</div>
`;
})
.join("")}
...
function levelTheoryTable(rows) {
if (!rows.length) return `<p class="stats-empty">아직 레벨별 통계가 없습니다.</p>`;
return `
<div class="table-wrap stats-scroll">
<table class="stats-rate-table">
<thead>
<tr>
<th>레벨</th>
<th>초심자</th>
...
(row) => `
<tr>
<th>${row.grade} ${formatInteger(row.level)}레벨</th>
${KIT_ORDER.map((kit) => `<td>${levelRateCell(row.kits && row.kits[kit])}</td>`).join("")}
</tr>
`,
)
.join("")}
...
function levelRateCell(item) {
const theoretical = Number(item && item.theoreticalGreatSuccessRate) || 0;
const attempts = Number(item && item.attempts) || 0;
const actual = Number(item && item.greatSuccessRate) || 0;
return `<span class="rate-cell">▌${formatPercent(theoretical, 1)}→${attempts ? formatPercent(actual, 1) : "-"}</span>`;
}
function difficultyList(rows) {
if (!rows.length) return `<p class="stats-empty">아직 구간별 통계가 없습니다.</p>`;
const maxAverage = Math.max(...rows.map((row) => Number(row.averageAttempts || 0)), 1);
return `
<div class="difficulty-list">
${rows.map((row) => difficultyRow(row, maxAverage)).join("")}
</div>
`;
}
function difficultyRow(item, maxAverage) {
const averageAttempts = Number(item.averageAttempts || 0);
const width = averageAttempts > 0 ? Math.max(4, (averageAttempts / maxAverage) * 100) : 0;
const ratio = maxAverage > 0 ? averageAttempts / maxAverage : 0;
const label = averageAttempts === 0 ? "집계 대기" : ratio >= 0.82 ? "지옥" : ratio <= 0.45 ? "쉬움" : "보통";
return `
<div class="difficulty-row">
<span>${item.label}</span>
<div class="difficulty-track"><b style="width:${width}%"></b></div>
<em>평균 ${averageAttempts ? formatNumber(averageAttempts, 1) : "-"}회</em>
<strong>${label}</strong>
</div>
`;
}
