...
<span>누적 참여</span>
<strong>${formatInteger(totalEvents)}건</strong>
<small>최근 ${formatInteger(stats.windowDays || 30)}일 기준</small>
</div>
<div class="stats-card">
<span>누적 시도</span>
<strong>${formatInteger(attempts)}회</strong>
<small>대성공 여부 확인에 사용된 키트 1회 단위</small>
</div>
<div class="stats-card">
<span>대성공</span>
<strong>${formatInteger(greatSuccesses)}건</strong>
<small>실측 대성공률 ${formatPercent(summary.greatSuccessRate || 0, 1)}</small>
</div>
</div>
<section class="stats-section">
<h3>키트별 이론 vs 실측 대성공률</h3>
...
<tbody>
${byKit.map((item) => statRateRow(item)).join("")}
</tbody>
</table>
</div>
</section>
...
${(stats.segmentStats || []).map((item) => difficultyCard(item)).join("")}
</div>
</section>
</div>
`;
}
function statRateRow(item) {
const actual = Number(item.greatSuccessRate || 0);
const theoretical = Number(item.theoreticalGreatSuccessRate || 0);
const delta = actual - theoretical;
return `
<tr>
<td><span class="stats-kit-label ${kitClass(item.kit)}"><i></i>${kitName(item.kit)}</span></td>
<td>${formatPercent(actual, 1)}</td>
<td>${formatPercent(theoretical, 1)}</td>
<td>${formatPercentagePoint(delta, 1)}</td>
<td>${formatInteger(item.attempts || 0)}회</td>
</tr>
`;
}
function attemptDistribution(rows) {
if (!rows.length) return `<p class="stats-empty">아직 대성공 시도 분포가 없습니다.</p>`;
const maxEvents = Math.max(...rows.map((row) => Number(row.events || 0)), 1);
return `
<div class="attempt-grid">
${KIT_ORDER
.map(
(kit) => {
const kitRows = rows.filter((row) => row.kit === kit);
return `
<div class="attempt-card">
<span class="stats-kit-label ${kitClass(kit)}"><i></i>${kitName(kit)}</span>
${
kitRows.length
? kitRows
.map((row) => {
const events = Number(row.events || 0);
const width = Math.max(4, (events / maxEvents) * 100);
return `
<div class="attempt-row">
<b>${formatInteger(row.successAttempt || 0)}회</b>
<div class="attempt-track"><span style="width:${width}%"></span></div>
<em>${formatInteger(events)}건</em>
</div>
`;
})
.join("")
: `<p class="stats-empty">기록 없음</p>`
}
</div>
`;
},
...
const score = Math.max(0, Math.min(1, Number(item.difficultyScore || 0)));
return `
<div class="difficulty-card">
<span>${item.label}</span>
<strong>${attempts ? formatPercent(item.greatSuccessRate || 0, 1) : "기록 없음"}</strong>
<div class="difficulty-track"><span style="width:${attempts ? Math.max(4, score * 100) : 0}%"></span></div>
<small>${attempts ? `${formatInteger(attempts)}회 시도, 이론 ${formatPercent(item.theoreticalGreatSuccessRate || 0, 1)}` : "집계 대기"}</small>
</div>
`;
}
