...
<div>
<dt>계산 방식</dt>
<dd>보유 키트 사용 가능 횟수를 상태에 포함한 유한 MDP입니다. 정확한 최대 SR 15 도달 확률에서 ${formatPercentagePoint(result.stats.probabilityTolerance || 0, 1)} 이내인 후보만 남긴 뒤, ${strategy === "supply" ? "수급량과 보유량을 함께 반영한 복합 비용이 낮은" : "보유량 압박과 기대 소모가 낮은"} 행동을 선택합니다. ${strategyBasis} 확률 전이는 정확 계산이며 반복 수렴 허용오차는 0입니다.</dd>
</div>
</dl>
<details class="validation-details">
<summary>검산 정보</summary>
<div class="validation-content">
<p>
추천 행동은 정확 MDP 전이값으로 계산됩니다. 몬테카를로 검산은 같은 정책을 무작위로 반복 실행해 결과가 직관적으로 맞는지 확인하는 보조 기능입니다.
</p>
<button class="secondary-button validation-button" type="button" data-validation-run>몬테카를로 검산 실행</button>
<p class="validation-result" data-validation-result>검산을 실행하면 SR 15 도달률이 여기에 표시됩니다.</p>
</div>
</details>
...
button.textContent = "검산 중";
if (output) output.textContent = `${runs.toLocaleString("ko-KR")}회 검산을 실행하고 있습니다.`;
try {
...
if (output) {
output.textContent = `몬테카를로 ${monteCarlo.runs.toLocaleString("ko-KR")}회 검산 결과의 SR 15 도달률은 ${formatPercent(
monteCarlo.successProbability,
2,
)}입니다.`;
}
button.textContent = "검산 다시 실행";
} catch (error) {
if (output) output.textContent = "검산 중 오류가 발생했습니다.";
button.textContent = "몬테카를로 검산 실행";
} finally {
