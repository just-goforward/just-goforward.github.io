...
const STRATEGY_PROBABILITY_TOLERANCE = {
single: 0.001,
supply: 0.01,
};
...
const KIT_INDEX = { blue: 0, purple: 1, yellow: 2 };
const WORST_CASE_USES_CACHE = new Map();
const STRATEGY_DEFAULT = "single";
const STRATEGY_META = {
single: {
label: "단일 목표",
description: "이 소장품의 SR 15 도달 확률을 가능한 유지하면서, 현재 보유 키트 기준으로 소모 부담이 낮은 선택을 계산합니다.",
},
supply: {
label: "수급량 고려",
description: "SR 15 도달 확률을 크게 깎지 않는 선에서, 수급량 75%와 현재 보유량 25%를 반영한 복합 비용이 낮은 선택을 계산합니다.",
},
};
const EXPECTED_28_DAY_GAIN = {
blue: 473.912,
purple: 55.808,
yellow: 24.736,
};
...
function isConvertStateNormalized(normalized) {
...
function failStateNormalized(state, kit) {
...
function capStockForState(normalized, stock, stats) {
const caps = {
...
if (!Number.isFinite(caps.blue) || !Number.isFinite(caps.purple) || !Number.isFinite(caps.yellow)) {
...
if (capped.blue === stock.blue && capped.purple === stock.purple && capped.yellow === stock.yellow) return stock;
...
function supplyCostScore(vector) {
return KIT_ORDER.reduce((sum, kit) => sum + vector[kit] / EXPECTED_28_DAY_GAIN[kit], 0);
}
function resourceCostScore(pressure, supplyCost, strategy) {
...
function probabilityToleranceForStrategy(strategy) {
return STRATEGY_PROBABILITY_TOLERANCE[normalizeStrategy(strategy)];
}
function withinProbabilityTolerance(successProbability, maxSuccessProbability, strategy) {
return maxSuccessProbability - successProbability <= probabilityToleranceForStrategy(strategy) + STRICT_EPSILON;
}
function compareEfficiency(a, b, strategy = STRATEGY_DEFAULT) {
if (strategy === "supply") {
if (Math.abs(a.resourceCost - b.resourceCost) > STRICT_EPSILON) return a.resourceCost - b.resourceCost;
...
function chooseEfficientCandidate(candidates, maxSuccessProbability, strategy) {
...
function compareByStrategy(a, b, maxSuccessProbability, strategy) {
const aEligible = withinProbabilityTolerance(a.successProbability, maxSuccessProbability, strategy);
const bEligible = withinProbabilityTolerance(b.successProbability, maxSuccessProbability, strategy);
...
edge.probability * success.successProbability +
(1 - edge.probability) * fail.successProbability;
const maxSuccessProbability = stateValue ? stateValue.maxSuccessProbability : successProbability;
const pressure = pressureScore(vector, initialUses);
const supplyCost = supplyCostScore(vector);
return {
...
while (count < limit && !isTerminalNormalized(state) && !isConvertStateNormalized(state) && stock[kit] > 0) {
...
const monteCarloRuns = Math.max(0, Math.floor(Number(input && input.monteCarloRuns) || 0));
...
const best = actionValues.find((candidate) => candidate.firstAction === bestAction) || actionValues[0];
const run = buildRecommendedRun(normalizedInput, mdp.actionFor);
const route = buildFailureRoute(normalizedInput, mdp.actionFor);
const monteCarlo =
monteCarloRuns > 0
? simulate(normalizedInput, mdp.actionFor, monteCarloRuns)
: {
runs: 0,
completed: 0,
