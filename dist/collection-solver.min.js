...
const PROBABILITY_PRESERVE_TOLERANCE = 0.001;
const STRATEGY_DEFAULT = "single";
const STRATEGY_META = {
single: {
label: "단일 목표",
description: "이 소장품만 SR 15까지 보낼 때, SR 15 도달 확률을 가능한 유지하면서 키트의 소모량을 줄입니다.",
},
supply: {
label: "수급량 고려",
description: "이 소장품의 SR 15 도달 확률을 크게 깎지 않는 선에서, 수급량이 적은 키트를 더 아낍니다.",
},
};
...
function withinProbabilityTolerance(successProbability, maxSuccessProbability) {
return maxSuccessProbability - successProbability <= PROBABILITY_PRESERVE_TOLERANCE + STRICT_EPSILON;
}
...
withinProbabilityTolerance(candidate.successProbability, maxSuccessProbability),
);
...
const aEligible = withinProbabilityTolerance(a.successProbability, maxSuccessProbability);
const bEligible = withinProbabilityTolerance(b.successProbability, maxSuccessProbability);
...
supplyCost: supplyCostScore(vector),
vector,
edge,
};
...
const stateValue = value(state, stock);
...
const monteCarloRuns = Math.max(0, Math.floor(Number(input && input.monteCarloRuns) || 12000));
...
exact: true,
tolerance: 0,
probabilityTolerance: PROBABILITY_PRESERVE_TOLERANCE,
...
FIXED_REQUIRED_EXP,
MAX_RELEVANT_USES,
PROBABILITY_PRESERVE_TOLERANCE,
STRATEGY_META,
